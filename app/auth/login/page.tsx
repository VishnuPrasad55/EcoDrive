    ...
    <div className="min-h-screen bg-background grid-bg flex items-center justify-center p-6">
      ...
      <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}
        ...
        <motion.div className="glass rounded-2xl p-8 border border-white/8" style={{ boxShadow: '0 0 60px rgba(0,240,122,0.06)' }}>
          ...
          <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.97 }} onClick={() => { setLoading(true); setTimeout(() => router.push('/dashboard'), 1000) }}
        ...
        <p className="text-center text-xs text-white/15 mt-6">Demo mode — no real auth required. Click Sign In to continue.</p>
      </motion.div>
    </div>
  )
}
